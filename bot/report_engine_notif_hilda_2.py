...
def _date_math(year_offset, days):
    if year_offset == 0:
        return f"now-{days}"
    else:
        return f"now-{year_offset}y-{days}"

def build_query_by_sendingtype(sendingtype,year_offset=1):
    return {
        "size": 0,
        "query": {
            "bool": {
                "must": [
                    {"term": {"sendingtype.keyword": str(sendingtype)}},
                    {
                        "range": {
                            "trxtime": {
                                "gte": _date_math(year_offset, "31d/d"),
                                "lte": _date_math(year_offset, "1d/d")
                            }
                        }
                    }
                ]
...
def build_query_avg_rt(year_offset=1):
    return {
        "size": 0,
        "query": {
            "range": {
...
def build_query_log(year_offset=1):
    return {
        "size": 0,
        "query": {
            "range": {
...
def cmd_report_engine_notif(chat_id, user_id, username, interval=None,year_offset=1):

    bot.send_message(chat_id, "Sedang diproses... ⏳")

    result_mvrk   = es_api_for_rqst(http, "GET", "/enginenotif-ttrx-*/_search", build_query_by_sendingtype(4, year_offset))
    result_sms    = es_api_for_rqst(http, "GET", "/enginenotif-ttrx-*/_search", build_query_by_sendingtype(1, year_offset))
    result_email  = es_api_for_rqst(http, "GET", "/enginenotif-ttrx-*/_search", build_query_by_sendingtype(2, year_offset))
    result_avg_rt = es_api_for_rqst(http, "GET", "/enginenotif-ttrx-*/_search", build_query_avg_rt(year_offset))
    result_log = es_api_for_rqst(http, "GET", "/log-enginenotif-*/_search", build_query_log(year_offset))
